	...
	void initializeDiagonal(){
		...
		if(orientation == 0){//left to right
			...
			for(int i = 0; i < num_lines; i++){
				start_pos[i] = new PVector(start_v + i*spacing, 0);
				current_pos[i] = new PVector(start_pos[i].x, 0);
				end_pos[i] = new PVector(i*spacing, p.height);
				lerp_pos_val[i] = 0f;
				lerp_pos_inc[i] = 0.01f + p.random(0.05f);
			}
		}
...
//		col = p.color(Vertices.p_h+var_h, Vertices.p_s+var_s, Vertices.p_b+var_b);
		...
		if(oscillating_space_cos)
			scaling = Vertices.p_spacing + PApplet.tan(beat)*Vertices.p_spacing_coeff;
		else
			scaling = Vertices.p_spacing + PApplet.cos(beat)*Vertices.p_spacing_coeff;
	...
	void display(){
		...
		p.pushMatrix();
		for(int i = 0; i < num_lines; i++){
			p.line(start_pos[i].x+(i-num_lines*0.5f)*scaling, start_pos[i].y, current_pos[i].x+(i-num_lines*0.5f)*scaling, current_pos[i].y);
		}
		p.popMatrix();
	}
}
